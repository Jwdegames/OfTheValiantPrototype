...
namespace Priority_Queue
{
        ...
        where TPriority : IComparable<TPriority>
    {
        ...
        private readonly GenericPriorityQueue<SimpleNode<TItem, TPriority>, TPriority> _queue;
        private readonly Dictionary<TItem, IList<SimpleNode<TItem, TPriority>>> _itemToNodesCache;
        private readonly IList<SimpleNode<TItem, TPriority>> _nullNodesCache;
        ...
        {
            _queue = new GenericPriorityQueue<SimpleNode<TItem, TPriority>, TPriority>(INITIAL_QUEUE_SIZE, priorityComparer);
            _itemToNodesCache = new Dictionary<TItem, IList<SimpleNode<TItem, TPriority>>>(itemEquality);
            _nullNodesCache = new List<SimpleNode<TItem, TPriority>>();
        }
        #endregion
        ...
        private SimpleNode<TItem, TPriority> GetExistingNode(TItem item)
        {
            ...
            IList<SimpleNode<TItem, TPriority>> nodes;
        ...
        private void AddToNodeCache(SimpleNode<TItem, TPriority> node)
        {
            ...
            {
                nodes = new List<SimpleNode<TItem, TPriority>>();
        ...
        public TItem Dequeue()
        {
            lock (_queue)
            {
                ...
                SimpleNode<TItem, TPriority> node = _queue.Dequeue();
        ...
        /// <summary>
        /// Removes the head node of the queue (node with minimum priority; ties are broken by order of insertion), and returns it.
        /// If queue is empty, throws an exception
        /// Custom Method by JWDE, not sure if it works O(log n)
        /// </summary>
        public SimpleNode<TItem, TPriority> DequeueNode()
        {
            lock (_queue)
            {
                if (_queue.Count <= 0)
        ...
        {
            SimpleNode<TItem, TPriority> node = new SimpleNode<TItem, TPriority>(item);
        ...
        public void Enqueue(TItem item, TPriority priority)
        {
            lock (_queue)
            {
                ...
                SimpleNode<TItem, TPriority> node = EnqueueNoLockOrCache(item, priority);
                nodes.Add(node);
            }
        }
        ...
        /// This queue automatically resizes itself, so there's no concern of the queue becoming 'full'.
        /// Duplicates and null-values are allowed.
        /// Modified by JWDE to add in a predecessor for the DijakstraCalculator Class O(log n)
        /// </summary>
        public void Enqueue(TItem item, TPriority priority, SimpleNode<TItem,TPriority> predecessor)
        {
            lock (_queue)
            {
                IList<SimpleNode<TItem, TPriority>> nodes;
        ...
        */
        ...
        public void Remove(TItem item)
        {
            lock (_queue)
            {
                SimpleNode<TItem, TPriority> removeMe;
                IList<SimpleNode<TItem, TPriority>> nodes;
                if (item == null)
                {
                    if (_nullNodesCache.Count == 0)
        ...
        /// <summary>
        /// Removes a node with an item from the queue.  The node does not need to be the head of the queue.  
        /// If the item is not in the queue, an exception is thrown.  If unsure, check Contains() first.
        /// If multiple copies of the item are enqueued, only the first one is removed. 
        /// Modified method made by JWDE, O(log n)
        /// </summary>
        public SimpleNode<TItem,TPriority> RemoveNode(TItem item)
        {
            lock (_queue)
            {
                SimpleNode<TItem, TPriority> removeMe;
        ...
        public void UpdatePriority(TItem item, TPriority priority)
        {
            lock (_queue)
            {
                SimpleNode<TItem, TPriority> updateMe = GetExistingNode(item);
                if (updateMe == null)
                {
        ...
        public TPriority GetPriority(TItem item)
        {
            lock (_queue)
            {
                SimpleNode<TItem, TPriority> findMe = GetExistingNode(item);
                if (findMe == null)
                {
        ...
        public bool IsValidQueue()
        {
            lock (_queue)
            {
                ...
                foreach (IList<SimpleNode<TItem, TPriority>> nodes in _itemToNodesCache.Values)
                {
                    foreach (SimpleNode<TItem, TPriority> node in nodes)
                    {
                ...
                foreach (SimpleNode<TItem, TPriority> node in _queue)
                {
